        ...
        if (!output.current_price) {
            try {
                ...
                if (!output.price.current_price) {
                    try {
                        output.price.current_price = this.geo.price_format($($('#rightCol .a-price')[0].children[0]).text());
                    } catch {
                        // continue regardless of error
                    }
                }
        ...
        for (let key in scrapingResult) {
            try {
                ...
                if (ratingSearch) {
                    ...
                    let total_reviews_string = '0';
                    if (ratingSearch.parent.parent.parent.next) {
                        total_reviews_string = ratingSearch.parent.parent.parent.next.attribs['aria-label'];
                    }
                    if (ratingSearch.parent.parent.next.next) {
                        total_reviews_string = ratingSearch.parent.parent.next.next.attribs['aria-label'];
                    }

                    scrapingResult[key].reviews.total_reviews = parseInt(
                        total_reviews_string.replace(/\,/g, ''),
                    );
